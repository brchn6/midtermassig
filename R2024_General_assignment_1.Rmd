...
DesnetiPlot<- functionForDesnetiPlot(longDataForPlot ) # the data must have col name log2Count and sample
...
#add col to fgens that will sum the number of genes that are between 4 and 9
fGenes <- fGenes %>% as.data.frame() 
fGenes$sum <- rowSums(fGenes) #sum the number of genes that are between 4 and 9
fGenes <- fGenes$sum >= 3 #create a logical vector of genes that have at least 3 samples with a value between 4 and 9

DATA <- DATA[fGenes,] #filter the data using the fGenes vector and store it in a tmpdata variable
...
functionToMake_tmp.data <- function(x) {
  tmp.data <- x
  # subtract the mean of each sample from all the sample values
  tmp.data[,grep("DC", names(x))] <- apply(x[,grep("DC", names(x))], 1, function(y){ y - mean(y)})
  return(tmp.data)
}
...
functionToMake_log2.norm <- function(x) {
  log2.norm <- x
  # subtract the mean of each sample from all the sample values
...
#this function is used to create a ggplot boxplot by inputting a dataframe
functio_for_ggplot_boxplot <- function(x,title) {
  ggplot(x,aes(x = sample, y = log2Count, fill = sample)) +
  geom_boxplot() +
  labs(title = title ,
       x = "Sample",
       y = "log2 gene expression levels") +
  theme_bw()
}
...
functio_for_ggplot_boxplot(DataForPlot1,"Boxplot of gene expression data of each sample BEFORE normalization BEFOR filtering") %>% print()

DataForPlot2 <- functionTolongerDataForPlot(tmp.data)
functio_for_ggplot_boxplot(DataForPlot2,"Boxplot of gene expression data of each sample BEFORE normalization AFTER filtering") %>% print()

DataForPlot3 <- functionTolongerDataForPlot(log2.norm)
functio_for_ggplot_boxplot(DataForPlot3,"Boxplot of gene expression data of each sample AFTER normalization AND filtering") %>% print()
...
```{r topvar-genes}
# to answer this Q i need to make a new var the hld the original data
...
#bring the variance to the first column
...
#select the top 2000 most varying genes
df <- df[1:2000,]
df %>% view()
```
```
...
```{r scale-samples}
#scale and center the subset of expression of genes and across samples
scaledData <- scale(df[,grep("DC", names(df))]) 
#convert the scaled data to a dataframe
scaledData <- as.data.frame(scaledData) 
 #add the geneSymbol column to the scaled data
scaledData <- cbind(df[,"geneSymbol"],scaledData)
#rename the first column to geneSymbol
names(scaledData)[1] <- "geneSymbol" 


# data is in long format, so we need to pivot it to wide format
longerDataForPlot <- scaledData[1:50,] %>%
  pivot_longer(
...
#plot a boxplot of the scaled expression of the first 50 genes
ggplot(longerDataForPlot, aes(x = sample, y = log2Count, fill = geneSymbol)) +
  geom_boxplot() +
  labs(title = "Boxplot of the scaled expression of the first 50 genes",
       x = "Sample",
       y = "Scaled log2 gene expression levels") +
  theme_bw() %>% print()


#this part im super nt sure about !!!!
```
...
kmeansClust <- kmeans(scaledData[,grep("DC", names(scaledData))], centers = 3)
#add the kmeans clusters to the scaled data
scaledData$cluster <- kmeansClust$cluster

ggplot(longerDataForPlot, aes(x = sample, y = log2Count, fill = factor(cluster))) +
  geom_point(position = position_jitterdodge(), alpha = 0.5) +
  labs(title = "Kmeans clusters of the top2000 most varying genes",
       x = "Sample",
       y = "Scaled log2 gene expression levels") + 
  theme_bw() %>% print()
...
```{r PCA-sample}
#compute the PCA on the log2-normalized expression data for the top2000 most varying genes
pca <- prcomp(scaledData[,grep("DC", names(scaledData))], scale = TRUE)
#add the PCA to the scaled data
scaledData$pca1 <- pca$x[,1]
scaledData$pca2 <- pca$x[,2]
#plot the PCA
pcaPlot <- ggplot(scaledData, aes(x = pca1, y = pca2, color = factor(cluster))) +
  geom_point(alpha = 0.5) +
  viridis::scale_color_viridis(discrete = TRUE) +
  labs(title = "PCA on the top2000 most varying genes",
       x = "PCA1",
       y = "PCA2") + 
  theme_bw() %>% print()
...
pcaPlot <- ggplot(scaledData, aes(x = pca1, y = pca2, color = factor(cluster))) +
  ...
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[1]^2 * 100, 2), "%")), 
            x = max(scaledData$pca1), y = max(scaledData$pca2), 
            hjust = 1, vjust = 1, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[2]^2 * 100, 2), "%")), 
            x = max(scaledData$pca1), y = min(scaledData$pca2), 
            hjust = 1, vjust = -0.5, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[3]^2 * 100, 2), "%")), 
            x = min(scaledData$pca1), y = max(scaledData$pca2), 
            hjust = -0.5, vjust = 1, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[4]^2 * 100, 2), "%")), 
            x = min(scaledData$pca1), y = min(scaledData$pca2), 
            hjust = -0.5, vjust = -0.5, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[5]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[6]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[7]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[8]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[9]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[10]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[11]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[12]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[13]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[14]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[15]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[16]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[17]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[18]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[19]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black") +
  geom_text(aes(label = paste0("Var: ", round(pca$sdev[20]^2 * 100, 2), "%")), 
            x = mean(scaledData$pca1), y = mean(scaledData$pca2), 
            hjust = 0, vjust = 0, size = 4, color = "black")

grid.arrange(pcaPlot, ncol = 1)
