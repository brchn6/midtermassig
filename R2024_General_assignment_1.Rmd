...
functio_for_ggplot_boxplot(DataForPlot1,"GeneExpration - Raw") %>% print()

DataForPlot2 <- functionTolongerDataForPlot(tmp.data)
functio_for_ggplot_boxplot(DataForPlot2,"GeneExpration - Filtterd") %>% print()

DataForPlot3 <- functionTolongerDataForPlot(log2.norm)
functio_for_ggplot_boxplot(DataForPlot3,"GeneExpration - Filtterd + norm") %>% print()
...
# and plot the box plot x= genes y= log2Count and fill = geneSymbol
...
kmeansClust <- kmeans(df[,grep("DC", names(df))], centers = 3)
#add the kmeans clusters to the scaled data
df$cluster <- kmeansClust$cluster
...
```{r PCA-sample}
#compute the PCA on the log2-normalized expression data for the top2000 most varying genes
pca <- prcomp(df[,grep("DC", names(df))], scale = TRUE)

#add the PCA to the scaled data
df$PC1 <- pca$x[,1]
df$PC2 <- pca$x[,2]

#plot the clusters and the PCA
p1 <- ggplot(df, aes(x = PC1, y = PC2, color = as.factor(cluster))) +
  geom_point() +
  labs(title = "PCA on samples gene expression profile - Kmeans-Clusters",
       x = "PC1",
       y = "PC2") +
  theme_dark() +
