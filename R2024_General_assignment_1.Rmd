...
DesnetiPlot<- functionForDesnetiPlot(log2.data) # the data must have col name log2Count and sample
...
#filter the data using the fGenes vector and store it in a tmpdata variable
filtered.data <- DATA[fGenes,] %>% na.omit() #without the na.omit the df will have 63679 rows and 14 columns 

#calculate the number of genes that were filtered out and retained
NumGensFilteredOut <- nrow(DATA) - nrow(filtered.data)
#calculate the number of genes that were retained
NumGensRetained <- nrow(filtered.data)


longerDataForPlot <- filtered.data %>% 
  pivot_longer(
    cols = c(grep("DC", names(DATA))), 
    names_to = "sample", 
    values_to = "log2Count"
...
ANSWER5 = "The number of genes that were filtered out is:  `r NumGensFilteredOut (21938)` and the number of genes that were retained is: `r NumGensRetained (6478)`"
```
...
```{r normalization}
funcrionForNormbyMean <- function(x){
  # subtract the mean of each sample from all the sample values
  tmp.data <- apply(filtered.data[,grep("DC", names(DATA))], 2, function(y) y - mean(y))
  # subtract from tmp.data its minimum value
  log2.norm <- sweep(tmp.data, 2, apply(tmp.data, 2, min), "-")
  return(log2.norm)
}
log2.norm <- funcrionForNormbyMean(filtered.data)
filtered_dataLog2norm <- filtered.data
filtered_dataLog2norm$log2Count <- log2.norm %>% as.vector()
```
...
# create a new variable with the data for plots
#call the function to create the plots

DataForPlot1 <- log2.data 
functio_for_ggplot_boxplot(DataForPlot1,"Boxplot of gene expression data of each sample BEFORE normalization AND filtering") %>% print()

DataForPlot2 <- filtered.data
...
DataForPlot3 <- filtered_dataLog2norm
